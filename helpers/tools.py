    ...
    def get_zips(xml_str: list, user_id=USER_ID) -> list:
        ...
        for num in range(0, len(output)):
            ...
            if output[num] == "0":
                ...
                # create variables to house address xml structures
                # create two variables to hold zip5 and zip4
                zips = root.findall("Address")
                zip_five = zips[counter].find('Zip5')
                zip_four = zips[counter].find('Zip4')

                # if zip5 and zip4 exist, then update output list with
                if zip_five and zip_four:

                    # force variable types on both variables.text methods
                    output[num] = str(zip_five.text) + str(zip_four.text)
